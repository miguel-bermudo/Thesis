...
\chapterQuote{
    \textit{``One learns from books and example only that certain things can be done. Actual learning requires that you do those things.''}\\
    }{--- Frank Herbert}

\chapterAbstract{R}{einforcement Learning encompasses all techniques which train intelligent agents on a simulated environment via rewards to their actions which modify said environment in some way. In this chapter we get an overview about how Reinforcement Learning operates and the different algorithms and techniques that have arisen. Section~\ref{sec:rl-intro} introduces the MDP description which encompasses all RL problems, which elements are found in every RL problem and how the interact and the evolution of the methods to perform it. Section~\ref{sec:rl-algorithms_and_techniques} introduces algorithms throughout the years and reward implementations that became standarized. Section~\ref{sec:rl-applications} shows some applications for Reinforcement Learning. Finally Section~\ref{sec:rl-summary} summarizes all information given.}
